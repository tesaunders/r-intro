...
Let's move the workshop [data file](data_raw/surveys_complete.csv) into the data_raw folder now.
...
In R, we can *assign* values to objects by using the assignment arrow `<-` (shortcut Alt+- on Windows/Linux and Option+- on Mac). Whatever is on the right of the arrow will be assigned to whatever is on the left. To create an object we provide a name for the object and assign a value. 
