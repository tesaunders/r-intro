...
You may have noticed we're using a .csv file, which is a plain text spreadsheet file. We're doing this because:
...
- Using a plain text format removes extraneous formatting that may have been entered when using something like Excel. Highlighting, notes in the margin, merged cells, and multiple tables in a single sheet may mean the file cannot be opened by software like R, and are all examples of things that should be handled differently to preserve data integrity (for example keeping metadata and notes in a README instead).
- Working with Excel files is possible in R but it requires installing and loading another package (popular options include readxl/writexl and openxlsx), which increases the dependencies in your code.
...
For example, we could join the `genus` and `species` names together into the full scientific name by using the `paste()` function. If we don't specify the `sep` argument (which separator to use between the data) it will default to a space which works for us here.
...
Another common example is to join multiple columns into dates. It’s good practice to keep the components of a date in separate columns until necessary, as in the surveys dataset. This is because programs like Excel automatically convert and change data interpreted as dates, and this can introduce errors. However, now that we are working in R, we can safely put together a date column.

Instead of using the general purpose `paste()` function, we can use the `make_date()` function from the `lubridate` package. This function will join the data and tell R that the resulting column is a special data type called 'date'. Converting date columns into date data types is useful when plotting and subsetting.
